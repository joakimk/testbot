...
class ServerTest < Test::Unit::TestCase
  ...
  def test_it_should_save_a_job_and_return_the_id
    post '/jobs', :files => 'spec/models/car.rb spec/models/house.rb'
    first_job = Job.first
    assert last_response.ok?    
    assert_equal first_job[:id].to_s, last_response.body
    assert_equal 'spec/models/car.rb spec/models/house.rb', first_job[:files]
  end
  
  def test_it_should_be_able_to_return_a_job_and_mark_it_as_taken
    job1 = Job.create :files => 'spec/models/car.rb'
    job2 = Job.create :files => 'spec/models/house.rb'    
    get '/jobs/next'
    assert last_response.ok?
    assert_equal job1[:id].to_s + ',' + "spec/models/car.rb", last_response.body
    assert job1.reload[:taken]
    assert !job2.reload[:taken]
  end
  
  def test_it_should_not_return_a_job_that_has_already_been_taken
    job1 = Job.create :files => 'spec/models/car.rb', :taken => true
    job2 = Job.create :files => 'spec/models/house.rb'    
    get '/jobs/next'
    assert last_response.ok?
    assert_equal job2[:id].to_s + ',' + "spec/models/house.rb", last_response.body
    assert job2.reload[:taken]    
  end
  ...
  def test_it_should_be_able_to_receive_the_results_of_a_job
    job = Job.create :files => 'spec/models/car.rb', :taken => true
